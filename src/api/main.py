...
async def lifespan(app: FastAPI):
    ...
    async with async_session() as session:
        ...
        # Create admin user "bake"
        admin_query = select(User).where(User.username == "bake")
        admin_result = await session.execute(admin_query)
        admin_existing = admin_result.scalar_one_or_none()
        ...
        if not admin_existing:
            admin_user = User(
                ...
                email="bake@local",
                username="bake",
                hashed_password=get_password_hash("bake"),
                role="admin",
                language="ru",
                stt_provider="google",
                tts_provider="google",
